...
export function VisitDetail({
  ...
  const tone = isActive ? toneColors("routeAlt") : toneColors("success");
  const scope = getVisitScope(visit);
  ...
  const checkOut = async () => {
    ...
    setMessage("");
    try {
      const updated = await sellerApi.checkOut(visit._id, {
      ...
      setError(
        toVietnameseError(
          err instanceof Error ? err.message : "Không check-out được",
        ),
      );
    } finally {
      setSubmitting(false);
      ...
      <View style={styles.page}>
        <View style={styles.header}>
          ...
          >
            <MaterialCommunityIcons
              name={
                isActive ? "map-marker-radius-outline" : "check-circle-outline"
              }
              size={21}
              color="#FFFFFF"
            />
        ...
        >
          <View style={styles.heroTop}>
            ...
            <View style={styles.heroPills}>
              <View
                style={[
                  styles.scopePill,
                  { backgroundColor: scope.bg, borderColor: scope.border },
                ]}
              >
                <MaterialCommunityIcons
                  name={scope.icon}
                  ...
                  color={scope.text}
                />
                <Text style={[styles.scopeText, { color: scope.text }]}>
                  {scope.label}
                </Text>
              </View>
              <View
          ...
          <Text style={styles.heroSub} numberOfLines={1}>
            {getRouteName(visit.route)}
          </Text>
          <View style={styles.heroStats}>
            <HeroMetric
              label="Check-in"
              value={shortDateTime(visit.checkInTime)}
            />
            <HeroMetric
              label="Check-out"
              value={
                visit.checkOutTime
                  ? shortDateTime(visit.checkOutTime)
                  : "Chưa có"
              }
            />
          </View>
        ...
        {isActive && onCreateOrder ? (
          <Pressable
            onPress={() => onCreateOrder(getCustomerId(visit.customer))}
            style={({ pressed }) => [
          ...
          >
            ...
            <View style={styles.createOrderText}>
              <Text style={styles.createOrderTitle}>
                Tạo đơn cho khách đang check-in
              </Text>
              <Text style={styles.createOrderDescription}>
                Đơn hàng sẽ tự gắn với {getCustomerName(visit.customer)}.
              </Text>
            </View>
            <MaterialCommunityIcons
              name="chevron-right"
        ...
        <View style={styles.card}>
          ...
          <Timeline>
            <TimelineItem
              ...
              color={bento.route}
              bg={bento.routeSoft}
              isLast={!visit.checkOutTime}
            >
              <TimelineContent
                title="Check-in"
                meta={shortDateTime(visit.checkInTime)}
            ...
            {visit.checkOutTime ? (
              <TimelineItem
                icon="logout"
                color={bento.success}
                bg={bento.successSoft}
                isLast
              >
                <TimelineContent
                  title="Check-out"
                  meta={shortDateTime(visit.checkOutTime)}
                  value={formatCoordinate(
        ...
        <View style={styles.infoGrid}>
          ...
          <InfoTile
            ...
            tone={visit.checkOutTime ? "success" : "muted"}
          />
          <InfoTile
            icon="clock-outline"
            label="Thời gian vào"
        ...
        <View style={styles.card}>
          ...
          <DetailRow
            label="Khách hàng"
            value={getCustomerName(visit.customer)}
          />
          <DetailRow label="Tuyến" value={getRouteName(visit.route)} />
          <DetailRow label="Loại ghé" value={scope.label} />
          <DetailRow
            label="Trạng thái"
            value={isActive ? "Đang ghé" : "Hoàn tất"}
          />
          <DetailRow
            label="Check-in"
            value={shortDateTime(visit.checkInTime)}
          ...
          <DetailRow
            label="Check-out"
            value={shortDateTime(visit.checkOutTime)}
          />
          ...
          <View style={styles.card}>
            <View style={styles.checkoutHeader}>
              ...
              >
                ...
                <Text style={styles.gpsButtonText}>
                  {gettingLocation ? "Đang lấy" : "GPS"}
                </Text>
              </Pressable>
            </View>
            <Text style={styles.sectionHint}>
              Lấy GPS hiện tại để hoàn tất lượt ghé.
            </Text>
            {position ? (
            ...
            <View style={styles.twoCols}>
              <MobileField
                ...
                onChangeText={setCheckOutLatitude}
              ...
              <MobileField
                ...
                onChangeText={setCheckOutLongitude}
                keyboardType="numeric"
            ...
            <MobileField
              ...
              multiline
              placeholder="Kết quả trưng bày, đặt hàng, phản hồi..."
            />
            <Pressable
              onPress={checkOut}
              disabled={submitting || !checkOutLatitude || !checkOutLongitude}
              style={({ pressed }) => [
                styles.checkoutButton,
                pressed && styles.pressed,
                (submitting || !checkOutLatitude || !checkOutLongitude) &&
                  styles.disabled,
              ]}
            >
...
function getVisitScope(visit: Visit) {
  if (visit.route) {
    return {
      label: "Trong tuyến",
  ...
  return {
    label: "Ngoài tuyến",
    icon: "map-marker-off-outline" as IconName,
    text: bento.warning,
...
const styles = StyleSheet.create({
  ...
  heroMetricValue: { color: bento.text, fontSize: 13, fontWeight: "700" },
  heroMetricLabel: {
  ...
  createOrderText: { flex: 1, minWidth: 0 },
  createOrderTitle: { color: bento.text, fontSize: 13, fontWeight: "700" },
  ...
  timelineTitle: { color: bento.text, fontSize: 13, fontWeight: "700" },
  timelineMeta: {
    color: bento.textSecondary,
    fontSize: 11,
    fontWeight: "700",
